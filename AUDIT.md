...
| F015 | medium | The minute bank (a single scalar) banked leftover minutes of ANY category but always redeemed/rolled them over as `ministry`, so banking credit minutes near the 55h cap misclassified them as ministry | verified | Fixed in `fa0d6bf` alongside the Ministry/Credit change — `quickLogTime` only banks ministry; credit is always logged whole. Verified in-browser: a 45m credit log wrote `{category:'credit',creditType:'LDC'}` with no round-up dialog and `fieldservice_minute_bank` untouched. |
