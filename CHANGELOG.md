...
> lock file together. `src/version.test.ts` fails if they ever disagree.
...
- **Faster at the door.** Logging a visit starts with two big buttons, **Talked** / **Not home**; the time is simply "Now" (tap *change* to edit it); and a return visit is one tap — **Tomorrow**, **+1 week**, **+2 weeks** or **Pick…**, at the same time of day. On a contact, **Log visit** comes first, and the new **📅 Schedule visit** sets a return visit without logging anything. A new contact asks only for the name, the street, where you are (captured automatically), what you talked about and the return visit — the rest waits under "More details". Adding a contact from a house keeps the street open underneath, so the next house is one tap away
- **A calmer Service tab.** "Change my goal" opens as a pop-up with **Cancel** — leaving it halfway no longer leaves the tab stuck. Auxiliary pioneering is set in that same role question; the card on Service is now a one-line summary with the application slip and a Change link. Editing a time entry uses the same quick form as logging one. The date shows once in the day view, and the extra "+ Log time" in Recent entries is gone. When catching up would mean more than 3 hours a day, the line says what's left and that every hour counts instead of a daily figure; a week target far above a normal week shows the hours you've logged, without the target
- **Getting started takes one screen.** After the opening animation there's a single welcome: three plain sentences about where your data lives (the full policy is a tap away), which of Publisher / Auxiliary / Regular pioneer you are, and your name if you like — then straight into Service, already set up. It used to be five steps. The guided tour is now five short stops (logging time, Today, People, your report, More) and lives in More, instead of popping up on first launch
- **More is half as long, in a sensible order:** your goal (with Change), backup (one status line), settings, the Streets & territories switch, help & feedback, then support — the tip card is one sentence now — and the legal notes last. The "Mark" theme is called **Navy**; credit-hour examples read as text instead of buttons that did nothing; the small screen-size line under the version is hidden
- **One word for each thing.** A contact's **status** (not "tag"), **visits** and **visit history** (not "calls"), **Not home** everywhere, **Open contact** everywhere, and **Log it** for time — "Submit" now means only handing in your report. There is one return-visit editor wherever you set or change a visit (it used to be three slightly different ones), with the same quick date chips
